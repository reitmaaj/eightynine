 ...
 * allocator used to destroy a value MUST be the allocator that allocated it.
 * A failed realloc leaves the original block owned by the value unchanged.
 */
...
/* ---- Views ---------------------------------------------------------------
 *
 * str89_view_init is the validation boundary: it validates the complete byte
 * sequence with libu89. After success, a view means validated Unicode text.
 * A view fabricated directly by a caller is the caller's responsibility.
 *
...
/* 1 when offset is a scalar boundary of s. Out-of-range offsets return 0 and
 * read no memory. */
int str89_view_is_boundary(str89_view s, size_t offset);

/* Subview [offset, offset + len). Both ends must be scalar boundaries.
 * Returns STR89_ERANGE for an out-of-range range and STR89_EBOUND for a split
 * scalar. On failure *out is unchanged. */
...
/* Copy a validated view into a fresh allocation. Empty input performs no
 * allocation. On failure *out is unchanged. */
int str89_from_view(str89 *out, const str89_alloc *alloc, str89_view src);

/* Copy an owning string. */
int str89_copy(str89 *out, const str89_alloc *alloc, const str89 *src);

/* Release the block and reset to the empty representation. Repeated calls are
 * safe. The allocator must match the one used to allocate. */
