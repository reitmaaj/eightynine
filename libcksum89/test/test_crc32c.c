/* test_crc32c.c - differential CRC-32C against the reference model over
...
static void check_bytes(void)
{
    ...
    for (i = 0; i < 256u; ++i)
    {
        ...
        cksum89_test_u32_at(cksum89_crc32c(one, 1), ref_crc32c(one, 1),
                            "crc32c one-byte", (unsigned long)i);
    }
}
...
static void check_lengths(const char *what)
{
    ...
    {
        cksum89_test_u32_at(cksum89_crc32c(corpus, len),
                            ref_crc32c(corpus, len), what, (unsigned long)len);
    }
}
...
void test_crc32c(void)
{
    check_bytes();
    fill_zero();
    check_lengths("crc32c zero");
    fill_ones();
    check_lengths("crc32c ones");
    fill_incrementing();
    check_lengths("crc32c incrementing");
    fill_decrementing();
    check_lengths("crc32c decrementing");
    fill_pseudorandom();
    check_lengths("crc32c pseudorandom");
}
