                             ...
                             raft89_id voted_for)
{
    ...
    if (action->u.hard_state.state.current_term != term)
    {
        return 0;
    }
                               ...
                               raft89_message *msg)
{
    ...
    msg->from = from;
    msg->to = to;
    msg->u.request_vote.term = term;
    msg->u.request_vote.last_log_index = last_index;
    msg->u.request_vote.last_log_term = last_term;
}
                     ...
                     raft89_size entry_count, raft89_message *msg)
{
    ...
    msg->from = from;
    msg->to = to;
    msg->u.append_entries.term = term;
    msg->u.append_entries.prev_log_index = prev_index;
    msg->u.append_entries.prev_log_term = prev_term;
    msg->u.append_entries.leader_commit = leader_commit;
...
void driver_store_effect(fake_store *store, const raft89_action *action)
{
    ...
    if (action->type == RAFT89_ACT_LOG_APPEND)
    {
        for (i = 0u; i < action->u.log_append.entry_count; ++i)
        {
            ...
            fake_store_put(store, entry->term, entry->index, entry->data,
                           entry->size);
        }
        return;
    }
