/* cat89_test_comma.c - comma category F ↓ G tests.
 *
 * Uses two identity functors 1_C over a finite groupoid C so that comma(1, 1)
 * is the arrow category of C: objects are the morphisms of C and morphisms are
 * commuting squares. Exercises object interning, square verification (commuting
 * squares accepted, non-commuting rejected) and the category laws over the
 * full arrow-category morphism set using an externally supplied equality. */
#include <stdlib.h>
...
/* External equality over comma(1,1): two comma morphisms are equal iff both
 * their C-component morphisms are equal under the base equality. */
struct ceq_ctx
{
...
static cat89_status ceq_mor_equal(void *ctx, const cat89_mor *f,
                                  const cat89_mor *g, int *out_equal)
{
    struct ceq_ctx *pc = ctx;
    cat89_status st;
    int el;
...
static void ceq_destroy(void *ctx)
{
    struct ceq_ctx *pc = ctx;
...
static cat89_status make_arrow_eq(cat89_category *arrow, cat89_eq *base,
                                  cat89_eq **out_eq)
{
    static const cat89_eq_ops ops = {NULL, ceq_mor_equal, ceq_destroy};
    struct ceq_ctx *pc;
    cat89_status st;

    pc = (struct ceq_ctx *)malloc(sizeof(*pc));
    if (pc == NULL)
    {
        return CAT89_NOMEM;
    }
...
int main(void)
{
    ...
    T_STATUS(cat89_comma_category_new(fid, gid, eq, NULL, &arrow), CAT89_OK);
    T_ASSERT(arrow != NULL);
    ...
    for (i = 0; i < n; i = i + 1)
    {
        ...
        T_STATUS(cat89_comma_obj_new(arrow, c, morphs[i], d, &objs[no]),
                 CAT89_OK);
        no = no + 1;
    }
    T_EQ_UL(no, n);
    ...
    for (i = 0; i < no; i = i + 1)
    {
        for (j = 0; j < no; j = j + 1)
        {
            for (u = 0; u < n; u = u + 1)
            {
                ...
                {
                    cand = NULL;
                    st = cat89_comma_mor_new(arrow, objs[i], objs[j], morphs[u],
                                             morphs[v], &cand);
                    if (st == CAT89_OK)
                    {
                        T_ASSERT(cand != NULL);
